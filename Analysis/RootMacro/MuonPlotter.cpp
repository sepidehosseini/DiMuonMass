...
/*
	...
	int nbins_Pt = 100 ;
*/
	TH1F *hist_InvMassMCP_Muon1 = new TH1F("hist_InvMassMCP_Muon1",";m_{#mu^{+}#mu^{-}};No.of J/#psi candidates",nbins_InvMass,2.8,3.8);
	hist_InvMassMCP_Muon1->SetLineWidth(1); hist_InvMassMCP_Muon1->SetLineColor(4);
	hist_InvMassMCP_Muon1->GetYaxis()->SetLabelSize(0.035);

	TH1F *hist_CosThetaMCP_Muon1 = new TH1F("hist_CosThetaMCP_Muon1"," ;cos#theta ;No.of J/#psi candidates",nbins_CosTheta,-1.,1.);
	hist_CosThetaMCP_Muon1->SetLineWidth(1); hist_CosThetaMCP_Muon1->SetLineColor(4);
	hist_CosThetaMCP_Muon1->GetYaxis()->SetLabelSize(0.04);
	TH1F *hist_PtMCP_Muon1 = new TH1F("hist_PtMCP_Muon1",";p_{t} ;No.of J/#psi candidates",nbins_Pt,0.,150.);
	hist_PtMCP_Muon1->SetLineWidth(1); hist_PtMCP_Muon1->SetLineColor(4);
	hist_PtMCP_Muon1->GetYaxis()->SetLabelSize(0.04);
	TH2F *hist_PtCosThetaMCP_os = new TH2F("hist_PtCosThetaMCP_os"," ;cos#theta ;p_{t} ",nbins_CosTheta,-1.,1.,nbins_Pt,0.,150.); //p_{t} - cos#theta (3.096 < m_{cut} < 3.1);cos#theta;p_{t} [GeV]
	...
	TH1F *hist_PtMCP_ls = new TH1F("hist_PtMCP_ls","p_{t} (3.096 < m_{cut} < 3.1)",nbins_Pt,0.,150.);
	...
	TH1F *hist_PtMCP_norm = new TH1F("hist_PtMCP_norm"," ;p_{t} [GeV];No.of J/#psi candidates",nbins_Pt,0.,150.);
	hist_PtMCP_norm->SetLineWidth(1); hist_PtMCP_norm->SetLineColor(1);
	hist_PtMCP_norm->GetYaxis()->SetLabelSize(0.035);
	TH2F *hist_PtCosThetaMCP_norm = new TH2F("hist_PtCosThetaMCP_norm"," ;cos#theta;p_{t} [GeV]",nbins_CosTheta,-1.,1.,nbins_Pt,0.,150.);//p_{t} - cos#theta (3.096 < m_{cut} < 3.1)
...
//	weight_mcp[0]=1; weight_mcp[1]=1; weight_mcp[2]=1 ;weight_mcp[3]=1;
...
//	weight_s5[0]=1; weight_s5[1]=1; weight_s5[2]=1; weight_s5[3]=1;
		...
		{
			tree_l5->GetEntry(i_entry);
			for (int  Muon1_index=0; Muon1_index<(m_mcpMuon1Index->size()); ++Muon1_index)
			{
				if ((m_mcpMuon1Index->at(Muon1_index))!=0)
				{
					hist_InvMassMCP_Muon1->Fill((m_mcpMuon1Index->at(Muon1_index)),weight_mcp[((2*m_polarizatione_l5+m_polarizationp_l5)+3)/2]);
					if (((m_mcpMuon1Index->at(Muon1_index))<3.1) && ((m_mcpMuon1Index->at(Muon1_index))>=3.09))
					{
						hist_CosThetaMCP_Muon1->Fill((m_mcpMuonCosTheta->at(Muon1_index)),weight_mcp[((2*m_polarizatione_l5+m_polarizationp_l5)+3)/2]);
						hist_PtMCP_Muon1->Fill((m_mcpMuonPt->at(Muon1_index)),weight_mcp[((2*m_polarizatione_l5+m_polarizationp_l5)+3)/2]);
						hist_PtCosThetaMCP_os->Fill((m_mcpMuonCosTheta->at(Muon1_index)),(m_mcpMuonPt->at(Muon1_index)),weight_mcp[((2*m_polarizatione_l5+m_polarizationp_l5)+3)/2]);
					}
					if ((((m_mcpMuon1Index->at(Muon1_index))<3.8) && ((m_mcpMuon1Index->at(Muon1_index))>=3.1)) || (((m_mcpMuon1Index->at(Muon1_index))<3.09) && ((m_mcpMuon1Index->at(Muon1_index))>=2.8)))
						++Num_Jpsis_os;
			}
//.....................................................................................................................				
				if ((m_mcpMuon1Index->at(Muon1_index))!=0)
				{
					hist_InvMassMCP_ls->Fill((m_mcpJpsiInvMass->at(Muon1_index)),weight_mcp[((2*m_polarizatione_l5+m_polarizationp_l5)+3)/2]);
					if (((m_mcpJpsiInvMass->at(Muon1_index))<3.1) && ((m_mcpJpsiInvMass->at(Muon1_index))>=3.09))
					{
						hist_CosThetaMCP_ls->Fill((m_mcpMuonCosTheta->at(Muon1_index)),weight_mcp[((2*m_polarizatione_l5+m_polarizationp_l5)+3)/2]);
						hist_PtMCP_ls->Fill((m_mcpMuonPt->at(Muon1_index)),weight_mcp[((2*m_polarizatione_l5+m_polarizationp_l5)+3)/2]);
						hist_PtCosThetaMCP_ls->Fill((m_mcpMuonCosTheta->at(Muon1_index)),(m_mcpMuonPt->at(Muon1_index)),weight_mcp[((2*m_polarizatione_l5+m_polarizationp_l5)+3)/2]);
					}
					if ((((m_mcpJpsiInvMass->at(Muon1_index))<3.8) && ((m_mcpJpsiInvMass->at(Muon1_index))>=3.1)) || (((m_mcpJpsiInvMass->at(Muon1_index))<3.09) && ((m_mcpJpsiInvMass->at(Muon1_index))>=2.8)))
						++Num_Jpsis_ls;
				}
			}
		}
		...
		for (int i_entry=0; i_entry<nEntries_l5; ++i_entry)
		{
			...
			for (int Muon1_index=0; Muon1_index<(m_mcpMuon1Index->size()); ++Muon1_index)
			{
				if ((m_mcpMuon1Index->at(Muon1_index))==0)
				{
					hist_InvMassMCP_norm->Fill((m_mcpJpsiInvMass->at(Muon1_index)),weight_mcp[((2*m_polarizatione_l5+m_polarizationp_l5)+3)/2]);
					if (((m_mcpJpsiInvMass->at(Muon1_index))<3.1) && ((m_mcpJpsiInvMass->at(Muon1_index))>=3.09))
					{
						hist_CosThetaMCP_norm->Fill((m_mcpMuonCosTheta->at(Muon1_index)),weight_mcp[((2*m_polarizatione_l5+m_polarizationp_l5)+3)/2]);
						hist_PtMCP_norm->Fill((m_mcpMuonPt->at(Muon1_index)),weight_mcp[((2*m_polarizatione_l5+m_polarizationp_l5)+3)/2]);
						hist_PtCosThetaMCP_norm->Fill((m_mcpMuonCosTheta->at(Muon1_index)),(m_mcpMuonPt->at(Muon1_index)),weight_mcp[((2*m_polarizatione_l5+m_polarizationp_l5)+3)/2]);
					}
				}
				if ((m_mcpMuon1Index->at(Muon1_index))!=0)
				{
					hist_InvMassMCP_norm->Fill((m_mcpJpsiInvMass->at(Muon1_index)),-norm * weight_mcp[((2*m_polarizatione_l5+m_polarizationp_l5)+3)/2]);
					if (((m_mcpJpsiInvMass->at(Muon1_index))<3.1) && ((m_mcpJpsiInvMass->at(Muon1_index))>=3.09))
					{
						hist_CosThetaMCP_norm->Fill((m_mcpMuonCosTheta->at(Muon1_index)),-norm * weight_mcp[((2*m_polarizatione_l5+m_polarizationp_l5)+3)/2]);
						hist_PtMCP_norm->Fill((m_mcpMuonPt->at(Muon1_index)),-norm * weight_mcp[((2*m_polarizatione_l5+m_polarizationp_l5)+3)/2]);
						hist_PtCosThetaMCP_norm->Fill((m_mcpMuonCosTheta->at(Muon1_index)),(m_mcpMuonPt->at(Muon1_index)),-norm * weight_mcp[((2*m_polarizatione_l5+m_polarizationp_l5)+3)/2]);
					}
				}
			}
		}
		...
		float int_InvMassMCP_norm = hist_InvMassMCP_norm->GetBinContent(30);
		cout << "Num_InvMassMCP_norm = " << int_InvMassMCP_norm << endl;
		float int_InvMassMCP_os = hist_InvMassMCP_Muon1->GetBinContent(30);
		cout << "Num_InvMassMCP_os = " << int_InvMassMCP_os << endl;
		float int_InvMassMCP_ls = hist_InvMassMCP_ls->GetBinContent(30);
		cout << "Num_InvMassMCP_os = " << int_InvMassMCP_ls << endl;


		float int_CosThetaMCP_os = hist_CosThetaMCP_Muon1->Integral(1,100);
		cout << "Num_CosThetaMCP_os = " << int_CosThetaMCP_os << endl;

		float int_CosThetaMCP_ls = hist_CosThetaMCP_ls->Integral(1,100);
		cout << "Num_CosThetaMCP_ls = " << int_CosThetaMCP_ls << endl;

		float int_CosThetaMCP_norm = hist_CosThetaMCP_norm->Integral(1,100);
		cout << "Num_CosThetaMCP_norm = " << int_CosThetaMCP_norm << endl;


		float int_PtMCP_os = hist_PtMCP_Muon1->Integral(1,100);
		cout << "Num_PtMCP_os = " << int_PtMCP_os << endl;

		float int_PtMCP_ls = hist_PtMCP_ls->Integral(1,100);
		cout << "Num_PtMCP_ls = " << int_PtMCP_ls << endl;

		float int_PtMCP_norm = hist_PtMCP_norm->Integral(1,100);
		cout << "Num_PtMCP_norm = " << int_PtMCP_norm << endl;
...
hist_InvMassMCP_Muon1->Sumw2();
	hist_InvMassMCP_Muon1->Draw("sames,hist");
	TLegend *InvMassMCP_os = new TLegend(0.45, 0.6, 0.7, 0.8);
	InvMassMCP_os->SetHeader("MC","l");
	InvMassMCP_os->AddEntry(hist_InvMassMCP_Muon1,"#mu^{+}#mu^{-}","l");
...
//	can_InvMassMCP_os->SaveAs((plots_dir+"test_InvMass50.pdf").c_str());
//	can_InvMassMCP_os->SaveAs((plots_dir+"test_InvMass50.C").c_str());
//	can_InvMassMCP_os->SaveAs((plots_dir+"test_InvMass50.eps").c_str());
...
hist_CosThetaMCP_Muon1->Sumw2();
	hist_CosThetaMCP_Muon1->Draw("sames,hist");
	TLegend *CosThetaMCP_os = new TLegend(0.45, 0.6, 0.7, 0.8);
	CosThetaMCP_os->SetHeader("Cos#theta","l");
	CosThetaMCP_os->AddEntry(hist_CosThetaMCP_Muon1,"#mu^{+}#mu^{-}","l");
...
//	can_CosThetaMCP_os->SaveAs((plots_dir+"test_CosTheta50.pdf").c_str());
//	can_CosThetaMCP_os->SaveAs((plots_dir+"test_CosTheta50.C").c_str());
//	can_CosThetaMCP_os->SaveAs((plots_dir+"test_CosTheta50.eps").c_str());
...
hist_PtMCP_Muon1->Sumw2();
	hist_PtMCP_Muon1->Draw("sames,hist");
	TLegend *PtMCP_os = new TLegend(0.45, 0.6, 0.7, 0.8);
	PtMCP_os->SetHeader("pt","l");
	PtMCP_os->AddEntry(hist_PtMCP_Muon1,"#mu^{+}#mu^{-}","l");
...
//	can_PtMCP_os->SaveAs((plots_dir+"test_Pt50.pdf").c_str());
//	can_PtMCP_os->SaveAs((plots_dir+"test_Pt50.C").c_str());
//	can_PtMCP_os->SaveAs((plots_dir+"test_Pt50.eps").c_str());
...
hist_PtCosThetaMCP_norm->Sumw2();
	...
	//can_PtCosThetaMCP_os->SaveAs((plots_dir+"test_Pt_cosThetaMCP50.pdf").c_str());
	//can_PtCosThetaMCP_os->SaveAs((plots_dir+"test_Pt_cosThetaMCP50.C").c_str());
	//can_PtCosThetaMCP_os->SaveAs((plots_dir+"test_Pt_cosThetaMCP50.eps").c_str());
*/
